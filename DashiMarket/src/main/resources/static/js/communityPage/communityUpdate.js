...
/* 삭제된 이미지 order를 기록할 Set 객체 */
const deleteSet = new Set();
const delBtn = document.getElementsByClassName("x-button");
...
for (let btn of mainBtn) {
    ...
    /* 이미 선택된 카테고리 효과 처리 */
    if(btn.dataset.category == initCategoryId) {

        btn.classList.add("text-green");

        mainCategory.value = initCategoryId;
        break;
    }
}
    ...
    .then(result => {
        ...
        if(result =='false'){
            ...
            setTimeout(() => {
                ...
                location.href = location.pathname.replace("/update", "") + location.search;
...
cancelBtn.addEventListener("click", () => {
    if(confirm("수정을 취소 하시겠습니까?")) {
        location.href = location.pathname.replace("/update", "");
    }
})